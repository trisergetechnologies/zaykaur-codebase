...
export default function PendingSellersPage() {
  ...
  const fetchPendingSellers = useCallback(() => {
    ...
    setLoading(true);
    axios
      .get(apiUrl("/api/v1/admin/sellers/pending"), {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        const d = res.data?.data;
        setData({ items: Array.isArray(d) ? d : [] });
      })
      .catch(() => setData({ items: [] }))
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Pending seller approvals" />
      <ComponentCard title="Pending sellers">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            ...
            {!loading && items.length === 0 && (
              <p className="py-6 text-center text-gray-500">No pending sellers.</p>
            )}
          </div>
        )}
      </ComponentCard>
